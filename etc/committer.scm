...
(define* (change-commit-message file-name old new #:optional (port (current-output-port)))
  ...
  (define version
    (and=> ((xpath:node-or
           ...
           first))
  (format port
          "guixcn: ~a: Update to ~a.~%~%* ~a (~a): Update to ~a.~%"
...
definition."
  (format port "guixcn: Add ~a.~%~%* ~a (~a): New variable.~%"
...
definition."
  (format port "guixcn: Remove ~a.~%~%* ~a (~a): Delete variable.~%"
          variable-name file-name variable-name))
...
ChangeLog entry."
  ...
  (let* ((message (trim message))
         ...
         (message/f (format #f "guixcn: ~a: ~a." variable-name message))
